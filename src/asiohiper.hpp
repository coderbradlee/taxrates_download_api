...
std::string retRabbitoverview = "";
//将返回的json赋值到这里，serverResource里处理完此json后赋值为空
...
static void new_conn(std::string url, GlobalInfo *g, std::string user = "admin", std::string password = "admin")
{
  ConnInfo *conn;
  CURLMcode rc;
  ...
  curl_easy_setopt(conn->easy, CURLOPT_HTTPAUTH, (long)CURLAUTH_BASIC);
  //curl_easy_setopt(conn->easy, CURLOPT_USERPWD, user+":"+password);
  std::string userpassword = user + ":" + password;
  //std::cout << "384\n" << userpassword << std::endl;
  curl_easy_setopt(conn->easy, CURLOPT_USERPWD, userpassword.c_str());
